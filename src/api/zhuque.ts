 ...
 * 朱雀官方送检通道 + 本地/官方校准
 *
 * 朱雀（matrix.tencent.com/ai-detect）有两条官方通道：
 *   A. 网页版（本模块）：粘贴或上传 txt/docx，最低 350 字，单次建议 ≤2000 汉字，
 *      有当日次数上限（2026-09 首页按钮显示「今日剩余 5 次」）；
 *      输出 AI 生成概率（百分比）+ 三档标签 + 红色高亮可疑片段。
 *      网页版有滑块风控，脚本点不动，也不该去解——所以本模块只做「备好文本 + 回填结果」。
 *   B. API 版（2026-09 起开放，见 api/detector.ts 的 ZHUQUE_OFFICIAL_DETECTOR）：
 *      EdgeOne Makers 内置模型 @makers/zhuque-text，Bearer Key，softmax_confidence 0~1，
 *      每月 50 万 token 免费。走的是「设置 → 对标检测器」，不经过本模块。
 * 因此本模块做三件事：
