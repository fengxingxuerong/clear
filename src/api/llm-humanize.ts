 ...
 *
 * 28 是**评委尺度**上的保守收手点，与 aiScore 那把尺子的边界
 * （AI_SCORE_HUMAN_MAX 27 / AI_SCORE_CUTOFF 29 / AI_SCORE_MACHINE_MIN 30，见
 * humanize-metrics.ts）**不是同一个量纲**，只是数值靠近、容易误读为同源。
 * 依据是本次四篇真跑的观测：落到 15 的稿子人眼与终审都算干净，而 45+ 的稿子改一轮
 * 也没更优；再考虑到评委分跨日不可比（同模型同稿历史上判过 16/24/38、本日判 46），
 * 28 只当作"到这里就别再折腾"的收手判据，不放宽任何绝对目标。
 */
