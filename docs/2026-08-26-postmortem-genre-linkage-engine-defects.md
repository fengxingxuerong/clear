...
> **2026-09-19 补记**：上面那行「A 套件 12/12」当天是真的，但同一份 A 套件在 2026-09-19 只剩 3/12，
> 而这段时间里 **A 套件的输入文本一个字没变**（冻在 `calibration-data-v2-genres.json` 里）。
> 也就是说这 9 行红全是评分器刻度移动造成的，与引擎无关——这反过来说明：本节把 A 套件当
> 「指标漂移没发生」的证据是无效的，它测的根本不是引擎。该套件已按 D/E 两组重写，
> 见 README「12 样本回归：漂移与退化分开拦」与 CHANGELOG v0.9.14 第 8 条。
> 好处是这条补记同时把责任范围钉死：`b3d68fe`（写档那次）到 HEAD 之间只有 5 个提交碰过
> `src/engine/humanize-metrics.ts`（`abbe9fa` / `34792ea` / `68b5e2f` / `773426f` / `50782b2`），
> 差值必然出在其中。**没有做二分去指认具体哪一个**——重写后的 D/E 两组都不依赖这个答案，
> D 组用「冻结文本 + 当前刻度实测」建线，E 组用同 seed 的引擎产物建线。
