...
> 现有 18 个送检点为人工抄录、仓库内不附页面截图等原始凭证，且 v0.8+ 重标定 aiScore 之后
> `scripts/calib-sanity.ts` 实测报 **14/18 点 x 轴漂移**（其自检口径为">3 即校准线过时信号"，
> 且当前不设退出码门禁）。下方"体裁换算线"只可当粗略参考，不可当官方分预报。
...
>
> ⚠️ **现状（2026-09-18 复核）**：`calib-sanity` 实测报 **14/18 点 x 轴漂移**（最凶的 6 点
> 是 `数据源x=0 → 当前引擎 60`，即 v0.9.10 清算的那族"`Number(null)===0`"塌缩锚点留下的污染），
> 其余为 v0.8+ 重标定 aiScore 后的整体位移。该脚本**只打印不拦**（退出码仍 0），所以这条线
> 目前处于"已过时但没人被拦"的状态；最后一次真实送检是 2026-09-13，v0.9.9 之后的三个
> release 没有新的官方数据。重拟合需要新的官方送检点，不要靠重生成基线把当前输出当真值。
